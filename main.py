...
for i in range(number):
    ...
    if discount:
        ...
        client_data[f'item_{i}'] = {'item':items,'quantity':str(quantity),
                                    ...
                                    'discount':str(discount_amount),'price after discount':str(price_after_discount),'vat':str(vat_price),'total':str(net_total)}
    else:
        discount_amount = 0
        price_after_discount = gross_total_price - discount_amount
...
for i in range(number):
    after_discount +=  int(client_data[f'item_{i}']['price after discount'])
...
pdf.cell(45,10,f'R{price_after_discount}',align='R', border=1, ln=1)
